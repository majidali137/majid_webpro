    ...
    <div className={`flex ${leftSide ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center`}>
      ...
      <div className="w-full lg:w-1/2">
        <motion.div
          ...
          whileHover={{ scale: 1.05, backgroundColor: '#2962FF' }} // Zoom-in effect and blue background on hover
          className="group bg-slate-800 text-white py-8 px-6 rounded-lg shadow-lg m-4 cursor-pointer hover:shadow-xl"
        >
          <h3 className="font-bold text-lg">{title}</h3>
          <h3 className="font-semibold text-[16px] text-gray-300">{name}</h3>
          {/* Period with hover effect linked to parent group hover */}
          <p className="text-sm font-semibold mb-2 mt-2 bg-slate-600 rounded-full px-4 py-2 max-w-[130px] group-hover:bg-[#0D47A1]">
            {period}
          </p>
          <p>{description}</p>
        </motion.div>
